                    ...
                    >
                        <div className="flex items-center justify-between gap-3">
                            <div className="min-w-0">
                                <p className="truncate font-semibold text-slate-900">{result.nom}</p>
                                {result.tipus === 'ITEM' && result.expo_nom && (
                                    <p className="truncate text-xs text-slate-500">Expo: {result.expo_nom}</p>
                                )}
                            </div>
                            {renderTypeBadge(result.tipus)}
                        </div>
                        {result.tipus === 'ITEM' && result.imatge_destacada_url && (
                            <p className="mt-1 text-xs text-slate-500">Té imatge destacada</p>
                        )}
                    </button>
                    ))
                ) : (
                    <p className="px-3 py-2 text-sm text-slate-500">No s'han trobat resultats.</p>
                )}
                </div>
            )}
        </section>
