...
function App() {
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden bg-gradient-to-br from-emerald-50 via-slate-100 to-orange-50 text-slate-900">
      ...
      <main className="relative mx-auto w-full max-w-6xl px-4 py-6 md:px-6 md:py-10">
        <header className="mb-4">
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-emerald-700">UXIA Web</p>
          <h1 className="mt-1 text-4xl font-black uppercase tracking-wide text-slate-900 md:text-6xl">UXIA Expos</h1>
        ...
        {status === 'ok' && (
          <section className="relative rounded-2xl border border-emerald-100 bg-white/90 p-4 shadow-[0_14px_35px_rgba(15,23,42,0.08)]">
            <label className="mb-2 block text-sm font-bold text-slate-700" htmlFor="expo-search">
              Buscador d'exposicions
            </label>
            <input
              className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-800 outline-none ring-emerald-200 transition focus:ring"
              id="expo-search"
            ...
            {shouldShowSuggestions && (
              <div className="absolute left-4 right-4 z-10 mt-2 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-lg">
                {suggestedExpos.length > 0 ? (
                  suggestedExpos.map((expo) => (
                    <button
                      className="block w-full border-b border-slate-100 px-3 py-2 text-left text-sm text-slate-800 transition hover:bg-slate-50"
                      key={expo.id}
        ...
        {selectedExpoId !== '' && itemsStatus === 'ok' && items.length > 0 && (
          <section className="mt-4 rounded-2xl border border-emerald-100 bg-white/90 p-4 shadow-[0_14px_35px_rgba(15,23,42,0.08)] md:p-5">
            ...
            <div className="relative overflow-hidden rounded-xl bg-slate-100">
              {currentItem?.imatge_destacada_url && currentItem?.id ? (
                <button className="block h-full w-full" onClick={() => setDetailItemId(currentItem.id)} type="button">
                  <img className="h-[320px] w-full object-cover md:h-[430px]" src={currentItem.imatge_destacada_url} alt={currentItem.nom} />
                </button>
              ) : (
                <div className="flex h-[320px] items-center justify-center px-4 text-center text-sm font-semibold text-slate-500 md:h-[430px]">
                  Sense imatge destacada
                </div>
              )}

              <div className="pointer-events-none absolute inset-x-0 top-0 bg-gradient-to-b from-slate-900/70 to-transparent p-3">
                <p className="text-xs font-semibold text-white/95 md:text-sm">{currentItem?.nom || 'Sense item'}</p>
              </div>

              <button
                aria-label="Item anterior"
                className="absolute left-2 top-1/2 -translate-y-1/2 rounded-full bg-slate-900/70 px-3 py-2 text-lg font-black text-white transition hover:bg-slate-900"
                onClick={goPrev}
                type="button"
              >
                &lt;
              ...
              <button
                ...
                className="absolute right-2 top-1/2 -translate-y-1/2 rounded-full bg-slate-900/70 px-3 py-2 text-lg font-black text-white transition hover:bg-slate-900"
                onClick={goNext}
                type="button"
              >
                &gt;
      ...
      {detailItemId !== null && (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-slate-900/55 p-4">
          ...
          <section className="relative z-10 w-full max-w-5xl rounded-2xl border border-emerald-100 bg-white p-4 shadow-[0_18px_40px_rgba(15,23,42,0.3)] md:p-5">
            <div className="mb-3 flex items-center justify-between">
              <h3 className="text-lg font-black text-slate-900 md:text-xl">Detall de l'item</h3>
              <button
            ...
            {detailStatus === 'ok' && detailItem && (
              <div className="grid grid-cols-1 gap-4 md:grid-cols-[1.2fr_1fr]">
                <div>
                  <div className="flex min-h-[260px] items-center justify-center overflow-hidden rounded-xl bg-slate-100">
                    {detailMainImage ? (
                      <img className="h-full max-h-[420px] w-full object-cover" src={detailMainImage} alt={detailItem.nom} />
                    ) : (
                  ...
                  {detailImages.length > 1 && (
                    <div className="mt-3 grid grid-cols-[1fr_auto_1fr] items-center gap-2">
                      <button
                        className="rounded-lg bg-emerald-700 px-3 py-2 text-sm font-bold text-white transition hover:bg-emerald-800"
                        onClick={goPrevDetailImage}
                        type="button"
                      >
                        Anterior
                      </button>
                      <p className="text-center text-sm font-bold text-slate-800">{detailImageIndex + 1} / {detailImages.length}</p>
                      <button
                        className="rounded-lg bg-orange-600 px-3 py-2 text-sm font-bold text-white transition hover:bg-orange-700"
                        ...
                        <button
                          className={
                            index === detailImageIndex
                              ? 'overflow-hidden rounded-lg border-2 border-emerald-400'
                          ...
                          type="button"
                        >
                          <img className="h-20 w-full object-cover" src={imageUrl} alt={detailItem.nom + ' ' + (index + 1)} />
                        </button>
                      ))}
                    </div>
                  )}
                ...
                <div>
                  ...
                  <h4 className="mt-1 text-2xl font-black text-slate-900">{detailItem.nom}</h4>
